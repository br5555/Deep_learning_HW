...
class FC(Layer):
    ...
    def forward(self, inputs):
        ...
        self.inputs = inputs
        return  np.matmul(self.inputs,np.transpose(self.weights)) + self.bias
    ...
    def backward_params(self, grads):
        ...
        grad_weights = np.matmul(np.transpose(grads), self.inputs)
        grad_bias = np.sum(grads, axis=0) / self.N
...
class ReLU(Layer):
    ...
    def forward(self, inputs):
        ...
        self.outputs = np.maximum(inputs,y)
        return self.outputs
    ...
    def backward_inputs(self, grads):
        ...
        return np.multiply(grads,self.outputs)
...
class SoftmaxCrossEntropyWithLogits():
    ...
    def forward(self, x, y):
        ...
        self.soft = exp_x_shifted / np.sum(exp_x_shifted, axis=1, keepdims=True)

#         self.soft = self.stable_softmax(x)
        error = 0
        for i in range(N):
            for j in np.nonzero(y[i])[0]:
                error +=  np.log( self.soft[i, j] ) 
        error /=-N 
        return error

        

#         logprobs = np.log(self.soft)

#         # cross entroy loss
#         loss = -np.mean(np.sum(np.multiply(y,logprobs), axis=1), axis=0, keepdims=True)
#         return loss
...
class L2Regularizer():
    ...
    def forward(self):
        ...
        """
        self.loss =0.5*(np.linalg.norm(self.weights) ** 2)
        return self.loss
...
#         self.weights *= self.weight_decay #forward or backward
