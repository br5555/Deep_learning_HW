...
# -*- coding: utf-8 -*-
"""
Created on Sun Jun 10 15:52:02 2018
...
with g1.as_default():
    ...
    hb1 = bias([Nh])
    a1 = tf.matmul(X1,w1)
    h0_prob = tf.sigmoid(hb1 + a1)
#    h0_prob = tf.reduce_prod(h0_prob,axis=1)
#    h0_prob =tf.zeros(shape=[tf.shape(h0_prob)[0], 100],dtype=tf.float32) + tf.reshape(h0_prob, [-1,1])
    h0 = sample_prob(h0_prob)
    h1 = h0

    for step in range(gibbs_sampling_steps):
        v1_prob = tf.nn.sigmoid(tf.matmul( h1, tf.transpose(w1)) +vb1)
#        v1_prob =  tf.reduce_prod(v1_prob, axis=1)
#        v1_prob = tf.zeros(shape=[tf.shape(v1_prob)[0], 784],dtype=tf.float32) + tf.reshape(v1_prob, [-1,1])
        v1 = sample_prob(v1_prob)
        a1 = tf.matmul(v1,w1)
        h1_prob = tf.sigmoid(hb1 + a1)
#        h1_prob =  tf.reduce_prod(h1_prob, axis=1)
#        h1_prob = tf.zeros(shape=[tf.shape(h1_prob)[0], 100],dtype=tf.float32) + tf.reshape(h1_prob, [-1,1])
...
with g3.as_default():
    ...
    h1_up =  sample_prob(h1_up_prob)
    v1_up_down_prob = tf.nn.sigmoid( tf.matmul(h1_up,w1_down) +vb1_down)
    v1_up_down = sample_prob(v1_up_down_prob)
    ...
    h2_down = h2_up
    for step in range(gibbs_sampling_steps):
        h1_down_prob = tf.nn.sigmoid(tf.matmul( h2down, tf.transpose(w2a)) +hb1_up)
    ...
    w2_negative_grad = tf.matmul(tf.transpose(h1_down),h2down)
    dw3 = (w2_positive_grad - w2_negative_grad) / tf.to_float(tf.shape(h1up)[0])
