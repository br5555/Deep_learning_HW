...
# -*- coding: utf-8 -*-
"""
Created on Sun Jun 10 15:59:55 2018
...
# definirajte skrivene varijable i pripadajući generator šuma
    ...
    z_log_sigma_sq = vae_layer(layer_e2, n_hidden_recog_2, n_z, 'z_sigma', act=tf.identity)
...
x_reconstr_mean = vae_layer(layer_d2, n_hidden_gener_2, n_input, 'output', act=tf.identity)  
...
with tf.name_scope('cost'):
    ...
    cost2 =  tf.exp(z_log_sigma_sq) + tf.square(z_mean) - 1 - z_log_sigma_sq
...
for epoch in range(n_epochs):
    ...
    for i in range(total_batch):
        ...
        opt, cos, cos1, cos2 = sess.run((optimizer, cost, cost1, cost2), feed_dict={x: batch_xs})
...
z_out= np.pad(z_out, (int((20-n_z)/2)), 'constant', constant_values=(0))
